...
export default async function LeadDetailPage({
  ...
  // Split this lead's appointments: open ("to do") vs. done (collapsed).
  const openAppointments = leadAppointments.filter(
    (a) => a.status === "scheduled" || a.status === "rescheduled",
  );
  const doneAppointments = leadAppointments.filter(
    (a) =>
      a.status === "completed" ||
      a.status === "no_show" ||
      a.status === "cancelled",
